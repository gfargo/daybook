...
export type BybitCsvRow = Record<string, string>;
...
function parseTimestamp(value: string): Date | undefined {
  ...
  // Bybit format: "2024-01-15 12:34:56" (UTC, no offset)
...
function hashRows(rows: BybitCsvRow[]): string {
  const stable = rows
    .map((row) =>
      Object.keys(row)
        .sort()
